...
const fetchCompanyById = async(id:number): Promise<Pick<Company, 'company_name'> | null> => {
  try {
    const sql = `SELECT company_name FROM Companies 
      WHERE company_id = ${id}`;
    const [rows] = await promisePool.execute<RowDataPacket[][] & Pick<Company, 'company_name'>[]>(sql);
    if (rows.length === 0) {
    return null;
  }
